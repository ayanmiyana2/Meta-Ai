...
def handle_start(message: Message):
    ...
    if user_id not in users:
        users[user_id] = {'premium_end': None, 'trial_used': False, 'points': 0, 'banned': False, 'pending_redeem': False, 'pending_confirm': False}
...
def handle_approve_fail(message: Message):
    ...
    if cmd == 'approved':
        bot.send_message(int(target_user_id), "Successfully Done ⚡️")
...
# Handle text messages for redeem
...
def handle_text(message: Message):
    ...
    save_data(PENDING_ORDERS_FILE, pending_orders)
    # Forward to admin
    notify_admin(f"New redeem from user {user_id} (Order #{order_id}): {message.text}\nUse /approved {order_id} or /failed {order_id}")
