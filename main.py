...
ADMIN_ID = int(os.environ.get('ADMIN_ID'))  # Set this in Render as your Telegram ID (e.g., 6324825537)
...
if __name__ == '__main__':
    ...
    # Run Flask
    app.run(host='0.0.0.0', port=PORT)@bot.message_handler(commands=['premium'])
def handle_premium(message: Message):
    user_id = str(message.from_user.id)
    if is_banned(user_id):
        return
